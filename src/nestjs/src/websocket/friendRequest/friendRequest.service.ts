...
export class WSFriendRequestService {
	...
	async getAllReqById(socket: Socket) {
		...
		const dest: number[] = [];
		for (let i = 0; i < tmp.length; i++) dest.push(tmp[i].meId);
	...
	async sendFriendReq(server: Server, socket: Socket, friend_id: number) {
		...
		const exist: boolean = await this.friendRequestService.alreadyFriend(
			me_id,
			friend_id,
		);
		const checkReverse = await this.friendRequestService.alreadySent(
			friend_id,
			me_id,
		);
	...
	async acceptFriendReq(server: Server, socket: Socket, friend_id: number) {
		...
		const friend_sock = this.wsSocket.getSocketId(friend_id);
		if (
			(await this.friendRequestService.alreadyFriend(friend_id, meId)) ==
			false
		)
			return;
		...
		server
			.to(friend_sock)
			.emit("friendReqStatus", FriendReqStatus.ALREADYFRIEND);
	...
	async friendReqStatus(socket: Socket, friend_id: number) {
		...
		if (
			...
			(await this.friendRequestService.alreadySent(meId, friend_id)) ==
			true
		)
			socket.emit("friendReqStatus", FriendReqStatus.SENT);
		else socket.emit("friendReqStatus", FriendReqStatus.NOTSENT);
	}
