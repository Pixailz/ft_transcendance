...
export class DBBlockedController {
	...
	// @Post()
	// create(@Request() req, @Body() post: DBBlockedPost) {
	...
	// @Get(":Blocked_id")
	// async getOne(@Request() req, @Param("Blocked_id") BlockedId: number) {
	// 	const userId = req.user.user_id;
	// 	return await this.dbBlockedService.returnOne(userId, BlockedId);
	// }

	// @Delete(":Blocked_id")
	// async delete(@Request() req, @Param("Blocked_id") BlockedId: number) {
	// 	const userId = req.user.user_id;
