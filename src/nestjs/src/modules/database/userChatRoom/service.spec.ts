...
describe("DBUserChatRoomService", () => {
  ...
  describe('testing create and returnOne', () => {
    it('[USER CHAT ROOM] TEST', async () => {
        ...
        let room = await chatRoomRepo.findOneBy({name: unit_room + "_name_BIS"})
  ...
  describe('[USER CHAT ROOM] delete', () => {
    it('should delete a room', async () => {
      const room = await chatRoomRepo.findOneBy({name: unit_room + '_name_BIS'}); 
      const user = await userService.returnOne(null, unit_user);
      await service.delete(user.id, room.id);
      await expect(service.returnOne(user.id, room.id)).rejects.toThrowError(
  ...
  describe('[USER CHAT ROOM] delete on cascade', () => {
    it('should delete a room', async () => {
      const roomId =await chatRoomService.create({name: unit_room + 'TEST_CASCADE'});  
      let userId = await userService.create({ftLogin: unit_user + 'CASCADE'});
      let post = {"isOwner": true, "isAdmin": true};
