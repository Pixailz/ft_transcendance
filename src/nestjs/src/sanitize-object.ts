...
export class Sanitize
{
	...
	{ if (user) return this.object(user, this.toKeepUser); else return (user); }
	...
	FriendRequest(req: FriendRequestEntity): FriendRequestEntity
	{
		...
		} as FriendRequestEntity); else return (req);
	}
	...
	Message(message: MessageEntity): MessageEntity
	{
		if (message)
		{
			var new_message: MessageEntity = this.object(message, this.toKeepMessage);
	...
	ChatRoom(room: ChatRoomEntity): ChatRoomEntity
	{
		if (room)
		{
			var new_room: ChatRoomEntity = this.object(room, this.toKeepChatRoom);
	...
	UserChatRoom(user_room: UserChatRoomEntity): UserChatRoomEntity
	{
		if (user_room)
		{
			var new_user_room: UserChatRoomEntity = this.object(user_room, this.toKeepUserChatRoom);
